	...
	return (
		<>
			<address { ...useBlockProps() }>
				...
				<RichText
					placeholder={ __( 'Enter address here...', 'valora-blocks' ) }
					tagName="p"
			...
			<InspectorControls>
				<PanelBody title={ __( 'Settings', 'valora-blocks' ) }>
					<PanelRow>
						<ToggleControl
							label={ __( 'Show SVG Icon', 'valora-blocks' ) }
							checked={ svgIcon }
							onChange={ ( value ) =>
								setAttributes( { svgIcon: value } )
							}
							help={ __( 'Display an SVG icon next to the address.', 'valora-blocks' ) }
						/>
					</PanelRow>
				</PanelBody>
			</InspectorControls>
